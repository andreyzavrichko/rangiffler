...
- .github - настройка интеграции с CI Github Actions
- .gradle - директория сборщика проекта Gradle
- .idea - системная директория IDE
- build - директория корневого проекта
- gradle - директория с gradle wrapper
- img - изображения для README.md
- logs - сохраненные логи микросервисов
- postgres - директория инициализации базы данных
- rangiffler-auth - микросервис регистрации и авторизации
- rangiffler-e-2-e-tests - проект с автотестами
- rangiffler-gateway - микросервис для отправки запросов к другим микросервисам
- rangiffler-geo - микросервис геолокации
- rangiffler-gql-client - фронтенд приложения
- rangiffler-grpc-common - микросервис grpc
- rangiffler-photo - микросервис работы с фотокарточками
- rangiffler-userdata - микросервис работы с пользователями
- selenoid - настройка браузеров
- wiremock - настройка моков
- .gitignore - настройка игнорирования файлов и директорий
- build.gradle - основной файл сборщика проекта
- docker.properties - файл настройки докера
- docker-compose.local.yml - локальный запуск проекта в докере
- docker-compose.yml - основной файл настройки запуска в докере
- docker-compose-dev.sh - shell скрипт для разворачивания приложения без запуска тестов
- docker-compose-e2e.sh - shell скрипт для разворачивания приложения, инфраструктуры и запуском тестов
- docker-compose-local.sh - shell скрипт локального разворачивания приложения в докере
- gradle.properties - настройка сборщика проекта
- gradlew - враппер сборщика проекта
- gradlew.bat - файл запуска враппера сборщика проекта
- localenv.sh - shell скрипт локального запуска приложения
- README.md - текущий файл
- settings.gradle - настройки сборщика проекта
...
- build - директория сборки тестового проекта
- graphql - директория со схемой graphql и файлами для генерации клиента
- api - директория для создания базового апи клиента
- condition - методы проверок условий
- config - настройка конфигурации
- data - клиент для работы с базой данных
- grpc - клиент для работы с grpc
- guicebinding - клиент для работы с инъекциями
- jupiter - директория с Junit Extensions и аннотациями
- mapper - методы для работы с таблицами
- model - модели для работы с апи, Allure, gql
- page - методы для работы с веб-страницами
- service - сервисы для работы с пользователями через апи и базу данных
- tests - тестовые методы для проверки слоя GraphQL, Grpc, Kafka, Web
- utils - утилитарные классы для генерации тестовых данных, работы со скриншотами и авторизацией
- resources/image - изображения для тестовых методов
- resources/META-ING - настройка работы с Junit Extensions и базой данных
- resources/tpl - настройка шаблонов для Allure Report
- jndi.properties - настройка работы с базой
- junit-platform.properties - настройка параллельного запуска автотестов
- logback.xml - настройка логирования
- spy.properties - настройка работы с базой
...
- graphql - запуск всех тестов на GraphQl
- geo - запуск тестов на геолокацию
- friends - запуск тестов на друзей
- invitations - запуск тестов на приглашения
- people - запуск тестов на людей
- photo - запуск тестов на фотокарточки
- user - запуск тестов на пользователя
- grpc - запуск всех grpc тестов
- statistic - запуск тестов на статистику
- auth - запуск тестов на авторизацию
- kafka - запуск тестов на Kafka
- web - запуск всех веб тестов
- register - запуск тестов на регистрацию
- map - запуск тестов на карту
- travel - запуск тестов путешествий
- userdata - запуск тестов с пользователями
- smoke - запуск основных сценариев
- regress - запуск регрессионных тестов (кроме smoke)
